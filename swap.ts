...
async function processSwaps() {
  ...
  for (const swapLog of swapLogs) {
    ...
    const ts = parsed - (parsed % 60)

    // Calculate USD value of the tokens sold by the given address
    const tokenPrice = getTokenPrice(
      POOL_ADDRESS_MAP[swapLog.pool],
      priceData[ts],
    )
    const tokenSold = ethers.utils.formatUnits(
      swapLog.tokensSold,
      POOL_ASSET_DECIMAL_MAP[swapLog.pool][parseInt(swapLog.soldId)],
    )
    const soldValue = (parseFloat(tokenSold) * tokenPrice.toNumber()) / 100

    console.log(
      `${swapLog.buyer} sold ${tokenSold} ${POOL_ADDRESS_MAP[swapLog.pool]}`,
    )
  ...
  // Set the rewards
  const rewardPerAddr = TOTAL_REWARD_TOKENS_BIGNUMBER.div(
    numberOfEligibleWallets,
  )
